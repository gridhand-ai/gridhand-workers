...
async function call({ modelString, clientApiKeys, systemPrompt, messages, maxTokens = 150, _workerName = null, tier = undefined }) {
    ...
    if (tier === 'director') {
        // Directors always use Opus regardless of modelString or OLLAMA_FIRST
        const directorModelId = 'claude-opus-4-7';
        const apiKey = getApiKey('anthropic', clientApiKeys);
        const _tag = _workerName ? `${_workerName}/tier-director` : 'tier-director/opus';
        return callAnthropic(directorModelId, apiKey, systemPrompt, messages, maxTokens, _tag);
    }
    ...
    if (tier === 'quality_escalate') {
        // Run Groq first; if quality fails escalate to Sonnet
        const groqConfig = PROVIDER_DEFAULTS.groq;
        ...
        try {
            ...
            console.warn(`[ai-client] quality_escalate: Groq quality check failed for ${_workerName || 'worker'} — escalating to Sonnet`);
        } catch (groqErr) {
            console.warn(`[ai-client] quality_escalate: Groq failed (${groqErr.message?.slice(0, 60)}) — escalating to Sonnet`);
        }
        // Escalate to Sonnet
        const anthropicKey = getApiKey('anthropic', clientApiKeys);
        const escalateTag = _workerName ? `${_workerName}/tier-quality-escalated` : 'tier-quality-escalated/sonnet';
        return callAnthropic('claude-sonnet-4-6', anthropicKey, systemPrompt, messages, maxTokens, escalateTag);
    }
