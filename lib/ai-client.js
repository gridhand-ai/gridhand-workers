...
//
// TIER ROUTING (preferred entry point — pass `tier` param to call()):
//   tier='quality'   → Always Groq llama-3.3-70b-versatile. Never Ollama.
//                       Use for: client-facing copy, outreach SMS/email,
//                       campaign feedback, content pipeline drafts.
//   tier='standard'  → Ollama first (if OLLAMA_BASE_URL set) → Groq 70b fallback.
//                       Use for: directors, scout summarization, internal reasoning.
//   tier='simple'    → Ollama first → Groq llama-3.1-8b-instant fallback (cheaper).
//                       Use for: classification, JSON pass/fail, memory ops.
//
// Provider used per call is logged: `[ai-client] provider=ollama model=...`
...
// Local Ollama model used when OLLAMA_BASE_URL is set and tier is standard/simple.
// Override via env: GRIDHAND_OLLAMA_MODEL=qwen2.5:14b (or whatever is pulled on the box).
const _ollamaLocalModel = process.env.GRIDHAND_OLLAMA_MODEL || 'qwen2.5:14b';

// True only when an Ollama tunnel/host is explicitly configured. We do NOT route
// to localhost in production by accident — only when MJ wires up the tunnel.
const _ollamaConfigured = Boolean(process.env.OLLAMA_BASE_URL || process.env.OLLAMA_URL);
...
async function call({ modelString, clientApiKeys, systemPrompt, messages, maxTokens = 150, _workerName = null, tier = undefined }) {
    ...
    // ── NEW TIER: 'quality' — always Groq 70b, never Ollama ───────────────────
    // Client-facing copy, outreach, campaign feedback. Quality > savings.
    if (tier === 'quality') {
        const groqConfig = PROVIDER_DEFAULTS.groq;
        const groqKey = getApiKey('groq', clientApiKeys);
        const _tag = _workerName ? `${_workerName}/tier-quality` : 'tier-quality/groq-70b';
        console.log(`[ai-client] provider=groq model=llama-3.3-70b-versatile tier=quality worker=${_workerName || 'unknown'}`);
    ...
    // ── NEW TIER: 'standard' — Ollama first → Groq 70b fallback ───────────────
    // Internal reasoning, directors, scout summarization.
    if (tier === 'standard') {
        const _tag = _workerName ? `${_workerName}/tier-standard` : 'tier-standard';
        if (_ollamaConfigured) {
            try {
                const ollama = PROVIDER_DEFAULTS.ollama;
                const result = await callOpenAICompat(
                    _ollamaLocalModel, null, ollama.baseUrl,
                    systemPrompt, messages, maxTokens, true, `${_tag}/ollama`
                );
                if (isQualityResponse(result)) {
                    console.log(`[ai-client] provider=ollama model=${_ollamaLocalModel} tier=standard worker=${_workerName || 'unknown'}`);
                    return result;
                }
                console.warn(`[ai-client] provider=groq model=llama-3.3-70b-versatile tier=standard worker=${_workerName || 'unknown'} (ollama quality fail)`);
            } catch (localErr) {
                console.warn(`[ai-client] provider=groq model=llama-3.3-70b-versatile tier=standard worker=${_workerName || 'unknown'} (ollama unreachable: ${localErr.message?.slice(0, 60)})`);
            }
        } else {
            console.log(`[ai-client] provider=groq model=llama-3.3-70b-versatile tier=standard worker=${_workerName || 'unknown'} (ollama not configured)`);
        }
        const groqConfig = PROVIDER_DEFAULTS.groq;
        const groqKey = getApiKey('groq', clientApiKeys);
        return callOpenAICompat(
    ...
    // ── NEW TIER: 'simple' — Ollama first → Groq 8B-instant fallback (free) ───
    // Classification, JSON pass/fail, memory summarization.
    if (tier === 'simple') {
        const _tag = _workerName ? `${_workerName}/tier-simple` : 'tier-simple';
        if (_ollamaConfigured) {
            try {
                const ollama = PROVIDER_DEFAULTS.ollama;
                const result = await callOpenAICompat(
                ...
                if (isQualityResponse(result)) {
                    console.log(`[ai-client] provider=ollama model=${_ollamaLocalModel} tier=simple worker=${_workerName || 'unknown'}`);
                    return result;
                }
                console.warn(`[ai-client] provider=groq model=llama-3.1-8b-instant tier=simple worker=${_workerName || 'unknown'} (ollama quality fail)`);
            } catch (localErr) {
                console.warn(`[ai-client] provider=groq model=llama-3.1-8b-instant tier=simple worker=${_workerName || 'unknown'} (ollama unreachable: ${localErr.message?.slice(0, 60)})`);
            }
        } else {
            console.log(`[ai-client] provider=groq model=llama-3.1-8b-instant tier=simple worker=${_workerName || 'unknown'} (ollama not configured)`);
        }
        const groqConfig = PROVIDER_DEFAULTS.groq;
        const groqKey = getApiKey('groq', clientApiKeys);
        return callOpenAICompat(
            'llama-3.1-8b-instant', groqKey, groqConfig.baseUrl,
            systemPrompt, messages, maxTokens, false, _tag
        );
    }
