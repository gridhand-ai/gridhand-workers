...
async function call({ modelString, clientApiKeys, systemPrompt, messages, maxTokens = 150, _workerName = null, tier = undefined }) {
    ...
    if (tier === 'director') {
        // Directors route to Groq — Anthropic credits unavailable
        const groqConfig = PROVIDER_DEFAULTS.groq;
        const apiKey = getApiKey('groq', clientApiKeys);
        const _tag = _workerName ? `${_workerName}/tier-director` : 'tier-director/groq';
        return callOpenAICompat('llama-3.3-70b-versatile', apiKey, groqConfig.baseUrl, systemPrompt, messages, maxTokens, false, _tag);
    }

    if (tier === 'specialist') {
        // Specialists always use Groq — no Anthropic fallback
        const specialistModelId = 'llama-3.3-70b-versatile';
        const groqConfig = PROVIDER_DEFAULTS.groq;
        const apiKey = getApiKey('groq', clientApiKeys);
        const _tag = _workerName ? `${_workerName}/tier-specialist` : 'tier-specialist/groq';
        return callOpenAICompat(specialistModelId, apiKey, groqConfig.baseUrl, systemPrompt, messages, maxTokens, false, _tag);
    }

    if (tier === 'quality_escalate') {
        // Run Groq first; if quality fails retry Groq (Anthropic unavailable)
        const groqConfig = PROVIDER_DEFAULTS.groq;
        ...
        try {
            ...
            console.warn(`[ai-client] quality_escalate: Groq quality check failed for ${_workerName || 'worker'} — retrying with Groq`);
        } catch (groqErr) {
            console.warn(`[ai-client] quality_escalate: Groq failed (${groqErr.message?.slice(0, 60)}) — retrying`);
        }
        // Retry with Groq — Anthropic unavailable
        const retryTag = _workerName ? `${_workerName}/tier-quality-retry` : 'tier-quality-retry/groq';
        return callOpenAICompat('llama-3.3-70b-versatile', groqKey, groqConfig.baseUrl, systemPrompt, messages, maxTokens, false, retryTag);
    }
    ...
    if (!providerConfig) {
        throw new Error(`Unknown AI provider: "${provider}". Supported: anthropic, groq, moonshot, ollama, openai`);
    }
...
function getModelLabel(modelString) {
    ...
    const labels = {
        ...
        'deepseek-reasoner': 'DeepSeek R1 (math/reasoning)',
