...
const LIMITS = {
    ...
    deepseek: {
        // DeepSeek V4 Pro — no hard rate limits published; set via env to warn early
        tpm:  parseInt(process.env.DEEPSEEK_TPM_LIMIT || '500000', 10),
        day:  parseInt(process.env.DEEPSEEK_DAY_LIMIT || '5000000', 10),
    },
    ollama: {
...
const _minute = {
    ...
    tokens: { groq: 0, anthropic: 0, deepseek: 0, ollama: 0 },
    requests: { groq: 0, anthropic: 0, deepseek: 0, ollama: 0 },
};
...
let _run = {
    id: null,
    startedAt: Date.now(),
    tokens:        { groq: 0, anthropic: 0, deepseek: 0, ollama: 0, moonshot: 0, perplexity: 0, openai: 0, total: 0 },
    input_tokens:  { groq: 0, anthropic: 0, deepseek: 0, ollama: 0, moonshot: 0, perplexity: 0, openai: 0 },
    output_tokens: { groq: 0, anthropic: 0, deepseek: 0, ollama: 0, moonshot: 0, perplexity: 0, openai: 0 },
    requests:      { groq: 0, anthropic: 0, deepseek: 0, ollama: 0, moonshot: 0, perplexity: 0, openai: 0, total: 0 },
    cost_usd:      { groq: 0, anthropic: 0, deepseek: 0, moonshot: 0, perplexity: 0, openai: 0 },
    warnings: [],
};
...
const _day = {
    ...
    tokens: { groq: 0, anthropic: 0, deepseek: 0, ollama: 0, total: 0 },
};
...
const PRICE_PER_1K = {
    ...
    deepseek:  0.00261, // V4 Pro standard: $1.74/M in + $3.48/M out blended (post-promo 2026-05-05)
    ollama:    0,       // free / local
};
...
function _tickWindow() {
    ...
    if (now - _minute.windowStart > 60_000) {
        ...
        _minute.tokens   = { groq: 0, anthropic: 0, deepseek: 0, ollama: 0 };
        _minute.requests = { groq: 0, anthropic: 0, deepseek: 0, ollama: 0 };
    }
}
...
function runSummary() {
    ...
    return {
        ...
        cost_usd: {
            ...
            deepseek:  +_run.cost_usd.deepseek.toFixed(4),
            total:     +(_run.cost_usd.groq + _run.cost_usd.anthropic + _run.cost_usd.deepseek).toFixed(4),
        },
        minute_window: {
...
function logUsage(provider, tag, inputTokens, outputTokens) {
    ...
    if (provider === 'deepseek') {
        const summary = runSummary();
        console.log(
            `[token-tracker] ${tag} | deepseek ${result.input}in+${result.output}out=${result.total}tok` +
            ` | run: ${summary.tokens.deepseek}tok ($${summary.cost_usd.deepseek}) | day: ${summary.day_tokens.deepseek}tok`
        );
    }
}
...
async function persistRun(supabase, runId) {
    const summary = runSummary();
    const providers = ['groq', 'anthropic', 'deepseek', 'ollama', 'moonshot', 'perplexity', 'openai'];
