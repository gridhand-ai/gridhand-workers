...
let _run = {
    ...
    requests: { groq: 0, anthropic: 0, ollama: 0, total: 0 },
    cost_usd: { groq: 0, anthropic: 0 },
    warnings: [],
};
...
function logUsage(provider, tag, inputTokens, outputTokens) {
    ...
    console.log(
        `[token-tracker] ${tag} | ${provider} ${result.input}in+${result.output}out=${result.total}tok` +
        ` | run: ${summary.tokens.total}tok ($${summary.cost_usd.total}) | day: ${summary.day_tokens.total}tok`
    );
}

module.exports = { record, resetRun, runSummary, logUsage, LIMITS };
