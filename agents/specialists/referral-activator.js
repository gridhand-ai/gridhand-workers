...
async function run(clients = []) {
  ...
  // Store offer structure (referral program state) per client into shared vault
  for (const r of reports) {
    if (r.clientId) {
      await vault.store(r.clientId, vault.KEYS.OFFER_STRUCTURE, {
        referralAskSent: r.status === 'action_taken',
        summary: r.summary || 'referral activation cycle complete',
        timestamp: Date.now(),
      }, 6, AGENT_ID).catch(() => {})
    }
  }
